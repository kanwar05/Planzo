...
test("booking conversations enforce membership and track unread, read, search, and deletion", async () => {
  const { booking, customerAuth, vendorAuth, outsiderAuth } = await setup();
  ...
  const sent = await customerAuth.agent.post(`/api/chat/conversations/${conversationId}/messages`).send({ text: "Hello 👋 about the wedding" });
...
test("authenticated sockets deliver realtime messages, typing, and seen receipts", async () => {
  const { booking, customerAuth, vendorAuth } = await setup();
  ...
  customerSocket.emit("message:send", { conversationId: conversation._id, text: "Realtime hello 😊" });
  assert.equal((await received).text, "Realtime hello 😊");
